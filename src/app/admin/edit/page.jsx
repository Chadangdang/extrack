...
export default function EditCategory() {
  ...
  const [selectedColor, setSelectedColor] = useState('#c5a3e8');
  ...
  const handleSaveChanges = () => {
    if (categoryName.trim() && selectedColor) {
      const colorName = colors.find((c) => c.value === selectedColor)?.name || 'Unknown';
  ...
  return (
    <div className="min-h-screen bg-[#F8F3ED] flex flex-col">
      ...
      <div className="w-full bg-[#E9D6BF] flex border-t-2 border-[#945C2B] fixed bottom-0">
        ...
        {/* Pencil (current page – active) */}
        <button
          className="flex-1 flex flex-col items-center justify-center py-4 bg-white border-t-4 border-[#945C2B]"
        >
